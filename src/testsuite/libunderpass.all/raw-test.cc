...
const std::vector<std::string> expectedGeometries = {
    ...
    "MULTILINESTRING((21.726084973 4.62036492836,21.726086573 4.62042742837,21.726001473 4.62042952837,21.726001473 4.62042952837,21.725999873 4.62036702836,21.726084973 4.62036492836))",
...
main(int argc, char *argv[])
{
    ...
    if (db->connect(dbconn + " dbname=underpass_test")) {
        ...
        if ( getWKTFromDB("relations", 16770204, db).compare(expectedGeometries[9]) == 0) {
            runtest.pass("MultiLinestring made of multiple ways (same changeset)");
        } else {
        ...
        if ( getWKTFromDB("relations", 16766330, db).compare(expectedGeometries[11]) == 0) {
            runtest.pass("MultiPolygon made of multiple ways (same changeset)");
        } else {
